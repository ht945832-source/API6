...
# Khởi tạo FastAPI
app = FastAPI(
    title="HỆ THỐNG ADMIN AI HOÀNG DZ", 
    description="Cổng điều khiển thủ công - Nhập phiên & Chốt kết quả"
)
bot = Bot(token=TOKEN)
...
class SystemState:
    def __init__(self):
        self.phien = 10001
        self.raw_data = deque(["Tài", "Xỉu", "Tài", "Xỉu"], maxlen=100) # Dữ liệu mồi tránh lỗi chia cho 0
        self.win = 0
        self.total = 0
...
def calculate_master_logic():
    ...
    # Logic xác suất dựa trên lịch sử + mã băm ngẫu nhiên
...
class ResultInput(BaseModel):
    phien_id: int
    ket_qua_thuc_te: str # Nhập "Tài" hoặc "Xỉu"

# --- ROUTES ---

@app.get("/", include_in_schema=False)
async def root():
    """Tự động chuyển hướng về trang điều khiển khi vào link chính"""
...
async def confirm_result(data: ResultInput):
    """
    ADMIN NHẬP KẾT QUẢ ĐỂ CHỐT PHIÊN VÀ RA DỰ ĐOÁN MỚI.
    """
    ...
    # 2. Gửi thông báo kết quả phiên vừa xong
    res_text = (
    ...
    # 3. Giả lập thời gian tính toán
    state.status = "ĐANG PHÂN TÍCH..."
    await asyncio.sleep(3)

    # 4. Nhảy sang phiên mới
    state.phien += 1
    ...
    # 5. Phát dự đoán mới lên Telegram
    ...
    pred_text = (
        ...
        f"📍 Admin: {ADMIN_ID}\n\n"
        f"🔥 <i>Liên hệ để mua thuật toán kéo 1-1 bao về bờ!</i>"
    )
    await send_to_tele(pred_text, kb)
    
    state.status = "CHỜ ADMIN CHỐT PHIÊN"
    return {"status": "Thành công", "phien_tiep_theo": state.phien, "du_doan_moi": pred}

# --- KHỞI CHẠY ---
if __name__ == "__main__":
    # Render yêu cầu port động
